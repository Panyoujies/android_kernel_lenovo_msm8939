...
#define FW_IMAGE_NAME "synaptics/PRstartup_fw_update.img"
#define BOE_IMAGE_NAME "synaptics/PRboe_startup_fw_update.img"
...
static int fwu_get_image_firmware_id(unsigned int *fw_id)
{
	...
	if (fwu->img.contains_firmware_id) {
		*fw_id = fwu->img.firmware_id;
		...
		strptr = strnstr(fwu->image_name, "PR", strlen(fwu->image_name));
		if (!strptr) {
			dev_err(rmi4_data->pdev->dev.parent,
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
		...
		retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				...
				printk("update touch firmware for LG\n");
		} else if (LCD_ID == 0x0) {
		retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				BOE_IMAGE_NAME, sizeof(BOE_IMAGE_NAME),
				sizeof(BOE_IMAGE_NAME));
				printk("update touch firmware for BOE\n");
		} else {
				printk("Unknow touch id\n");
		}
...
exit:
	...
	if (update_f) {
		...
		retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(custom_key));
		mdelay(100);
		for (cal_loop = 0; cal_loop < 50; cal_loop++) {
			retval = synaptics_rmi4_reg_read(rmi4_data,
				0x400,
				&custom_key,
				sizeof(custom_key));
			...
			if ((custom_key == 0x00) || (custom_key == 0x06))
				break;
			mdelay(1000);
			if (cal_loop == 14)
