...
static ssize_t synaptics_rmi4_calibration_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);

static ssize_t synaptics_rmi4_calibration_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_incharger_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_incharger_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(calibration, (S_IRUGO | S_IWUGO),
			synaptics_rmi4_calibration_show,
			synaptics_rmi4_calibration_store),
	__ATTR(touch_debug, S_IWUGO,
			synaptics_rmi4_show_error,
			synaptics_rmi4_touch_debug_store),
	__ATTR(glove_on,  (S_IRUGO | S_IWUGO),
			synaptics_rmi4_glove_show,
			synaptics_rmi4_glove_store),
	__ATTR(in_charger,  (S_IRUGO | S_IWUGO),
			synaptics_rmi4_incharger_show,
			synaptics_rmi4_incharger_store),
};
...
static void synaptics_rmi4_incharger_set(struct synaptics_rmi4_data *rmi4_data, unsigned int on)
{
	...
	if (on)
		...
		state &= ~(1 << 5);
	dev_info(rmi4_data->pdev->dev.parent, "incharge state = 0x%2x\n", state);
		...
		struct device_attribute *attr, char *buf)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			0x109,
			&incharger_state,
			sizeof(incharger_state));
	return snprintf(buf, PAGE_SIZE, "noise state is %s\n", (1 == incharger_state)? "normal": "suppressed");
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	sscanf(buf, "%u", &temp);
	synaptics_rmi4_incharger_set(rmi4_data, temp);
...
static ssize_t synaptics_rmi4_glove_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char glove_state;
	int retval;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);

	if (rmi4_data->flash_on)
		return snprintf(buf, PAGE_SIZE, "In flash mode\n");
	retval = synaptics_rmi4_reg_read(rmi4_data,
			0x1b,
			&glove_state,
			sizeof(glove_state));
	return snprintf(buf, PAGE_SIZE, "glove state = 0x%02x\n", glove_state);
}
...
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	sscanf(buf, "%u", &glove_mode);
	if (glove_mode)
		rmi4_data->enable_glove_mode = 1;
	else
		rmi4_data->enable_glove_mode = 0;
	if (!rmi4_data->flash_on)
		synaptics_rmi4_f12_glove(rmi4_data, glove_mode);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	int retval;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	unsigned char custom_key, cal_ret;
	int cal_loop;
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			&custom_key,
			sizeof(custom_key));
	custom_key = custom_key | 0x01;
	printk("calibration keys calibration status = 0x%x!!!\n", custom_key);
	retval = synaptics_rmi4_reg_write(rmi4_data,
			0x135,
			&custom_key,
			sizeof(custom_key));
	mdelay(100);
	for (cal_loop = 0; cal_loop < 500; cal_loop++) {
		retval = synaptics_rmi4_reg_read(rmi4_data,
				0x10E,
				&custom_key,
				sizeof(custom_key));
		printk("calibration keys loop = %d,calibration status = 0x%x!!!\n", cal_loop, custom_key);
		cal_ret = custom_key & 0x3;
		if ((cal_ret == 0x00) || (cal_ret == 0x02)) {
			printk("Touch calibration completed!!!\n");
			break;
		}
		mdelay(1000);
		if (cal_loop == 499)
	...
	if (cal_ret == 0x00)
		cal_result = 0;
	else if (cal_ret == 0x02)
		cal_result = 2;
	else if ((cal_ret == 0x1) || (cal_ret == 0x3))
		cal_result = 1;
	else
		cal_result = 2;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	gesture_wake = rmi4_data->enable_wakeup_gesture;
...
flash_prog_mode:
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
		rmi4_data->enable_wakeup_gesture = gesture_wake;
	else
		rmi4_data->enable_wakeup_gesture = false;

	printk("=======wake gesture = %d,f11 = %d\n", rmi4_data->enable_wakeup_gesture, rmi4_data->f11_wakeup_gesture);
