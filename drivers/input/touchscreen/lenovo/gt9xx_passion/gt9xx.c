...
#define INPUT_PHYS_NAME "goodix_ts/input0"
...
static int gtp_glove_flag = 0;
static int letter = 0 ;
static int count_i2c_power = 0;
...
s32 gtp_glove_send_cfg(struct i2c_client *client)
{
    s32 ret = 2;
    ...
    else if (ts->pnl_glove_init_error)
    {
        GTP_INFO("Error occured in glove init_panel, no config sent");
        return 0;
    }
...
static void gtp_touch_up(struct goodix_ts_data* ts, s32 id)
{
    ...
    pr_debug("[wj]Touch id[%2d] release!\n", id);
#else
   input_report_key(ts->input_dev, BTN_TOUCH, 0);
   // input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, 0);
    //input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, 0);
    input_mt_sync(ts->input_dev);
    pr_debug("[wj]Touch release!\n");
#endif
}
...
static irqreturn_t goodix_ts_irq_handler(int irq, void *dev_id)
{
    ...
    //pr_debug("[wj]goodix_ts_irq_handler.");
...
{
	 if(ts->disable_gpios){
    		GTP_INFO("ahe Cancel gtp_int_sync because of disable_gpios");
   		return;
   	 }
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	/* This reset sequence will selcet I2C slave address */
	 if(ts->disable_gpios){
    		GTP_INFO("ahe Cancel gtp_reset_guitar because of disable_gpios");
   		return;
   	 }
...
static s8 gtp_enter_sleep(struct goodix_ts_data * ts)
{
	...
	if(ts->disable_gpios==false){
		...
		GTP_INFO("gpio_disable %d ",ts->disable_gpios);
	}
	return ret;
err_gpio_configure:
	if (ts->ts_pinctrl) {
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
	...
	if(gtp_wakeup_flag && (doze_status!=DOZE_DISABLED) ){
		...
		if(ts->disable_gpios==true){
			...
			dev_err(&ts->client->dev, "ahe gpio ok %d \n",ts->disable_gpios);
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    u8 cfg_glove_info_group1[] = CTP_CFG_GROUP_GLOVE_1;
    u8 cfg_glove_info_group2[] = CTP_CFG_GROUP_GLOVE_2;
    u8 cfg_glove_info_group3[] = CTP_CFG_GROUP_GLOVE_3;
    u8 cfg_glove_info_group4[] = CTP_CFG_GROUP_GLOVE_4;
    u8 cfg_glove_info_group5[] = CTP_CFG_GROUP_GLOVE_5;
    u8 cfg_glove_info_group6[] = CTP_CFG_GROUP_GLOVE_6;
    ...
    u8 *send_glove_cfg_buf[] = {cfg_glove_info_group1, cfg_glove_info_group2, cfg_glove_info_group3,
                        cfg_glove_info_group4, cfg_glove_info_group5, cfg_glove_info_group6};
    u8 cfg_glove_info_len[] = { CFG_GROUP_LEN(cfg_glove_info_group1),
                          CFG_GROUP_LEN(cfg_glove_info_group2),
                          CFG_GROUP_LEN(cfg_glove_info_group3),
                          CFG_GROUP_LEN(cfg_glove_info_group4),
                          CFG_GROUP_LEN(cfg_glove_info_group5),
                          CFG_GROUP_LEN(cfg_glove_info_group6)};
    ...
    GTP_DEBUG("Glove Config Groups\' Lengths: %d, %d, %d, %d, %d, %d", 
        cfg_glove_info_len[0], cfg_glove_info_len[1], cfg_glove_info_len[2], cfg_glove_info_len[3],
        cfg_glove_info_len[4], cfg_glove_info_len[5]);
    ...
    ts->gtp_glove_cfg_len = cfg_glove_info_len[sensor_id];
    GTP_INFO("CTP_GLOVE_CONFIG_GROUP%d used, config length: %d", sensor_id + 1, ts->gtp_glove_cfg_len);
    ...
    if (ts->gtp_glove_cfg_len < GTP_CONFIG_MIN_LENGTH)
    {
        GTP_ERROR("Glove Config Group%d is INVALID CONFIG GROUP(Len: %d)! NO Config Sent! You need to check you header file CFG_GROUP section!", sensor_id+1, ts->gtp_glove_cfg_len);
        ts->pnl_glove_init_error = 1;
    }
...
#endif
    {
        ...
        if (ret == SUCCESS)
        {
            ...
            if (opr_buf[0] < 90)    
            {
                ...
                glove_cfg_version = send_glove_cfg_buf[sensor_id][0];
                send_cfg_buf[sensor_id][0] = 0x00;
                send_glove_cfg_buf[sensor_id][0] = 0x00;
                ts->fixed_cfg = 0;
            }
    ...
    if(ts->pnl_glove_init_error != 1)
    {
        memset(&config_glove[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
        memcpy(&config_glove[GTP_ADDR_LENGTH], send_glove_cfg_buf[sensor_id], ts->gtp_glove_cfg_len);
    }
    ...
    config_glove[RESOLUTION_LOC]     = (u8)GTP_MAX_WIDTH;
    config_glove[RESOLUTION_LOC + 1] = (u8)(GTP_MAX_WIDTH>>8);
    config_glove[RESOLUTION_LOC + 2] = (u8)GTP_MAX_HEIGHT;
    config_glove[RESOLUTION_LOC + 3] = (u8)(GTP_MAX_HEIGHT>>8);
    
    if (GTP_INT_TRIGGER == 0)  //RISING
    {
        config_glove[TRIGGER_LOC] &= 0xfe; 
    }
    else if (GTP_INT_TRIGGER == 1)  //FALLING
    {
        config_glove[TRIGGER_LOC] |= 0x01;
    }
    ...
    glove_check_sum = 0;
    for (i = GTP_ADDR_LENGTH; i < ts->gtp_glove_cfg_len; i++)
    {
        glove_check_sum += config_glove[i];
    }
    config_glove[ts->gtp_glove_cfg_len] = (~glove_check_sum) + 1;
...
#endif
    {
        ...
        config_glove[GTP_ADDR_LENGTH] = glove_cfg_version;
        check_sum = 0;
        for (i = GTP_ADDR_LENGTH; i < ts->gtp_cfg_len; i++)
        {
            check_sum += config[i];
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    ts->input_dev->phys = INPUT_PHYS_NAME;
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "name=gt%s; types=%s; fw_num=%s; cfg_version=%02x\n",GTP_IC_VERSION,vendor_id,GTP_FW_VERSION,grp_cfg_version);
}
#if GTP_GESTURE_WAKEUP 
...
#ifdef GTP_GLOVE
static ssize_t gtp_glove_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n",gtp_glove_flag);
}

static ssize_t gtp_glove_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long val;
	int error;
	...
	if(val == 1)
	{
		...
		{
            if(ts->powerdown){
	        GTP_ERROR("Glove Send config error, power down.");
	        return count;
            }
		    gtp_glove_flag = 1;
                ...
                gtp_irq_disable(ts);
                msleep(10);
                ret = gtp_glove_send_cfg(ts->client);
                if (ret < 0)
                {
                    GTP_ERROR("Glove Send config error.");
                }
                msleep(200);//add by wengjun1 for ic self-study.
                GTP_ERROR("[wj]Switch to glove mode.");
	        gtp_irq_enable(ts); 
            msleep(10);
            #endif
	...
	}else
	{
		if(gtp_glove_flag)
		{
		    ...
		    gtp_glove_flag = 0;
                ...
                GTP_ERROR("[wj]Switch to normal mode.");
                msleep(200);//add by wengjun1 for ic self-study.
	            gtp_irq_enable(ts); 
                msleep(10);
            #endif
