	...
	if (DOZE_ENABLED == gesture_doze_status) {
		...
		if (ret == 0) {
			if (doze_buf[0] == 0 || !QUERYBIT(gestures_flag, doze_buf[0])) {
				GTP_INFO("Sorry, this gesture[0x%02X] has been disabled.", doze_buf[0]);
                gesture_enter_doze();
				doze_buf[0] = 0x00;
...
#define IO_VERSION                   "V1.2-20140709"
...
#ifdef CONFIG_COMPAT
static long gt1x_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg32)
{
