...
void gt1x_power_reset(void)
{
	...
	if (resetting || update_info.status) {
		return;
	}
...
s32 gt1x_touch_event_handler(u8 * data, struct input_dev * dev, struct input_dev * pen_dev)
{
	...
	/* read the remaining coor data 
        * 0x814E(touch status) + 8(every coordinate contains 8bytes data) * touch num + 
        *keycode + checksum
        */
	if (touch_num > 1) {
    ...
    do{
       ...
       len = cfg_lens[sensor_id];
        if (len == 0 || len != gt1x_cfg_length) {
            ...
            GTP_DEBUG("Length of config is incorrect.");
            return -1;
        }
    	...
    	if (gt1x_int_type == 0) {	/* RISING  */
    		cfg[TRIGGER_LOC] &= 0xfe;
    	} else if (gt1x_int_type == 1) {	/* FALLING */
    		cfg[TRIGGER_LOC] |= 0x01;
    	}
...
static void gt1x_init_charger(void)
{
    ...
    if (gt1x_parse_chr_cfg(gt1x_version.sensor_id) < 0) {
        GTP_DEBUG("Error occured when parse charger config.");
    }
}
    ...
    do{
       ...
       *len = cfg_lens[sensor_id];
    	...
    	/*cfg[RESOLUTION_LOC] = (u8) gt1x_abs_x_max;
    	      ...
    	      cfg[RESOLUTION_LOC + 2] = (u8) gt1x_abs_y_max;
    	      cfg[RESOLUTION_LOC + 3] = (u8) (gt1x_abs_y_max >> 8);
    	      do not change the resolution of smartcover module
            */
    	if (gt1x_int_type == 0) {	/* RISING  */
    		cfg[TRIGGER_LOC] &= 0xfe;
    	} else if (gt1x_int_type == 1) {	/* FALLING */
...
static int gt1x_smart_cover_init(void)
{
    ...
    if (!sysfs_rootdir) {
        // this kobject is shared between modules, do not free the memory when error occur
    ...
    GTP_INFO("SmartCover module init successful.");
    return 0;
