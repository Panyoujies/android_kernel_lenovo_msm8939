...
static void gt1x_ts_work_func(struct work_struct *work)
{
	...
	if (gt1x_halt) {
		GTP_DEBUG("Ignore interrupts after gt1x suspend...  ");
		return;
	}
...
#if GTP_ICS_SLOT_REPORT
        input_mt_init_slots(input_dev, 16, 0);      // for Kernel Version >= 3.8
...
static int __init gt1x_ts_init(void)
{
    s32 ret;
	GTP_DEBUG_FUNC();
	GTP_INFO("[wj]GTP gt11xx driver installing...");
    ...
    GTP_INFO("[wj]gt11xx ret is %d.",ret);
	return ret;
}
