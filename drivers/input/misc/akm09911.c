...
static bool akm_self_test(struct sensors_classdev *sensors_cdev);
...
static ssize_t akm_raw_data_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	int flag;
	int rawx, rawy, rawz;
...
static struct device_attribute akm_compass_attributes[] = {
	...
	__ATTR(raw_xyz, 0444, akm_raw_data_show, NULL),
	__ATTR(self_test, 0444, self_test_show, NULL),	
#if AKM_DEBUG_IF
				...
				struct akm_compass_data *akm)
{
	...
	dev_info(dev, "auto_report:%d, use_hrtimer:%d\n", akm->auto_report, akm->use_hrtimer);
...
//	} while ((ret == -EAGAIN) && (--count));
	...
	if ((ret != -EAGAIN) && STATUS_ERROR(tmp)) {
	...
	if (ret == -EAGAIN) {
		dev_warn(&akm->i2c->dev, "not ready, report last data\n");
		mag_x = akm->last_x;
		mag_y = akm->last_y;
		mag_z = akm->last_z;
	} else {
...
//	printk("akm:%ld\n", (long int)akm->delay[MAG_DATA_FLAG]);
