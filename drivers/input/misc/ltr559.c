...
static int ltr559_ps_enable(struct i2c_client *client, int on)
{
	...
	if (on) {
		...
		msleep(WAKEUP_DELAY);//10ms
               ...
               ltr559_set_ps_threshold(client, LTR559_PS_THRES_LOW_0, 2047);//data->platform_data->prox_hsyteresis_threshold);
               ltr559_set_ps_threshold(client, LTR559_PS_THRES_UP_0, 2046);//data->platform_data->prox_threshold);
               #endif
	} else {
...
static int ltr559_als_enable(struct i2c_client *client, int on)
{
	...
	if (on) {
		...
		cancel_work_sync(&data->als_work.work);
		hrtimer_start(&data->als_timer,
			ns_to_ktime((data->platform_data->als_poll_interval-0) * 1000000UL),
			HRTIMER_MODE_REL);
	} else {
		//cancel_delayed_work_sync(&data->als_work);
		hrtimer_cancel(&data->als_timer);
		cancel_work_sync(&data->als_work.work);
...
static enum hrtimer_restart ltr_als_timer_func(struct hrtimer *timer)
{
	struct ltr559_data *data = container_of(timer, struct ltr559_data, als_timer);

	queue_work(data->work_queue, &data->als_work.work);
	hrtimer_forward_now(&data->als_timer,
		ns_to_ktime((data->platform_data->als_poll_interval-0) * 1000000UL));
...
int ltr559_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	data->work_queue = alloc_workqueue("ltr559_work",
	...
	data->als_timer.function = ltr_als_timer_func;
