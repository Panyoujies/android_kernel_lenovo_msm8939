...
/*lenovo-sw caoyi1 add for Gsensor calibration 20150420 begin*/
#define LENOVO_GSENSOR_CALI
/*lenovo-sw caoyi1 add for Gsensor calibration 20150420 end*/
/*
...
struct kionix_accel_driver {
	/* regulator data */
	bool power_on;
/*lenovo-sw caoyi1 modify for do not use vdd---L17 ldo 20150508 begin*/
//	struct regulator *vdd;
/*lenovo-sw caoyi1 modify for do not use vdd---L17 ldo 20150508 end*/
...
/*lenovo-sw caoyi1 modify for testmode ATTR 20150421 begin*/
	.name = "gsensor",//kionix-accel
/*lenovo-sw caoyi1 modify for testmode ATTR 20150421 begin*/
	.vendor = "kionix",
	.version = 1,
			...
			if (err < 0) {
				...
				z = ((s16)
				     le16_to_cpu(accel_data.accel_data_s16
						 [acceld->axis_map_z])) >>
				    acceld->shift;
/*lenovo-sw caoyi1 modify for Gsensor calibration 20150422 begin*/
#ifdef LENOVO_GSENSOR_CALI
		
				acceld->accel_data[acceld->axis_map_x] = (acceld->negate_x ? -x : x)  - acceld->cali_data[0];
				acceld->accel_data[acceld->axis_map_y] = (acceld->negate_y ? -y : y)  - acceld->cali_data[1];
				acceld->accel_data[acceld->axis_map_z] = (acceld->negate_z ? -z : z)  - acceld->cali_data[2];

/*lenovo-sw caoyi1 update raw data for sys/class/sensors/gsensor/sensors_ftm_raw 20150421 begin */
					acceld->cdev.raw_data.x = acceld->accel_data[acceld->axis_map_x];
					acceld->cdev.raw_data.y = acceld->accel_data[acceld->axis_map_y];
					acceld->cdev.raw_data.z = acceld->accel_data[acceld->axis_map_z];
/*lenovo-sw caoyi1 update raw data for sys/class/sensors/gsensor/sensors_ftm_raw 20150421 end */
#else
				acceld->accel_data[acceld->axis_map_x] =
				    (acceld->negate_x ? -x : x) +
				    acceld->accel_cali[acceld->axis_map_x];
...
#endif
/*lenovo-sw caoyi1 modify for Gsensor calibration 20150422 end*/
				if (atomic_read(&acceld->accel_input_event)
					> 0) {
					input_report_abs(acceld->input_dev,
							 ABS_X,
...
/*lenovo-sw caoyi1 modify for fix compile error 20150420 begin*/
	//return snprintf(buf, sizeof(buf)+1, "%d\n",
		       //atomic_read(&acceld->accel_enabled) > 0 ? 1 : 0);
		return sprintf(buf, "%d\n",atomic_read(&acceld->accel_enabled) > 0 ? 1 : 0);
/*lenovo-sw caoyi1 modify for fix compile error 20150420 end*/
...
/*lenovo-sw caoyi1 modify for fix compile error 20150420 begin*/
	enable = simple_strtoul(buf, &buf2, 10);
...
/*lenovo-sw caoyi1 modify for fix compile error 20150420 begin*/
	//return snprintf(buf, sizeof(buf)+1, "%d %d %d\n", x, y, z);
...
/*lenovo-sw caoyi1 add for Gsensor calibration 20150420 begin*/
#ifdef LENOVO_GSENSOR_CALI
static ssize_t kxtj9_fast_calibration_x_show(struct device *dev,
		struct device_attribute *attr, char *buf) 
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kionix_accel_driver *acceld = i2c_get_clientdata(client);
	...
	calibration[0] = acceld->cali_data[0];
	calibration[1] = acceld->cali_data[1];
	calibration[2] = acceld->cali_data[2];

	read_unlock(&acceld->rwlock_accel_data);
	return sprintf(buf, "%d\n", calibration[0]);
}

static ssize_t kxtj9_fast_calibration_x_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	...
	if (error < 0)
		return error;
	acceld->cali_data[0] = data;	
	acceld->cdev.cal_result.offset_x = data;
	return  count;
}

static ssize_t kxtj9_fast_calibration_y_show(struct device *dev,
		struct device_attribute *attr, char *buf) 
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kionix_accel_driver *acceld = i2c_get_clientdata(client);
...
static ssize_t kxtj9_fast_calibration_y_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	...
	if (error < 0)
		return error;
	acceld->cali_data[1] = data;	
	acceld->cdev.cal_result.offset_y = data;
	return  count;
}

static ssize_t kxtj9_fast_calibration_z_show(struct device *dev,
		struct device_attribute *attr, char *buf) 
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kionix_accel_driver *acceld = i2c_get_clientdata(client);
...
static ssize_t kxtj9_fast_calibration_z_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	...
	if (error < 0)
		return error;
	acceld->cali_data[2] = data;	
	acceld->cdev.cal_result.offset_z = data;
	return  count;
}
#endif
/*lenovo-sw caoyi1 add for Gsensor calibration 20150420 end*/
...
/*lenovo-sw caoyi1 modify for testmode ATTR 20150421 begin*/
static int kxtj9_sensors_self_test(struct sensors_classdev *sensors_cdev) 
{
...
static int kxtj9_sensors_calibrate_set(struct sensors_classdev *sensors_cdev,
					int axis, int apply_now)
{
	...
	if (0 == axis)
	{
		acceld->cali_data[0] = (s16)apply_now;
		acceld->cdev.cal_result.offset_x = apply_now;
	}
	else if (1 == axis)
	{
		acceld->cali_data[1] = (s16)apply_now;
		acceld->cdev.cal_result.offset_y = apply_now;
	}
	else if (2 == axis)
	{
		acceld->cali_data[2] = (s16)apply_now;
		acceld->cdev.cal_result.offset_z = apply_now;
	}
	else
		return -1;
...
static ssize_t kxtj9_acc_raw_show(struct device *dev,
		struct device_attribute *attr, char *buf) 
{
	int  x, y, z;
	struct i2c_client *client = to_i2c_client(dev);
...
/*lenovo-sw caoyi1 add for Gsensor calibration 20150420 begin*/
#ifdef LENOVO_GSENSOR_CALI
static DEVICE_ATTR(fast_calibration_x, S_IRUGO | S_IWUGO,
			kxtj9_fast_calibration_x_show, kxtj9_fast_calibration_x_store);
static DEVICE_ATTR(fast_calibration_y, S_IRUGO | S_IWUGO,
			kxtj9_fast_calibration_y_show, kxtj9_fast_calibration_y_store);
static DEVICE_ATTR(fast_calibration_z, S_IRUGO | S_IWUGO,
			kxtj9_fast_calibration_z_show, kxtj9_fast_calibration_z_store);
#endif
/*lenovo-sw caoyi1 add for Gsensor calibration 20150420 end*/
/*lenovo-sw caoyi1 modify for testmode ATTR 20150421 begin*/
static DEVICE_ATTR(gsensor_ftm_raw, S_IRUGO | S_IWUGO,
			kxtj9_acc_raw_show, NULL);
/*lenovo-sw caoyi1 modify for testmode ATTR 20150421 end*/
...
enable_delay:
/*lenovo-sw caoyi1 modify for shorten the time after powered up 20150428 begin*/
//Power up to communication,it takes 10ms before it is ready for IIC communication
	msleep(30);//130
/*lenovo-sw caoyi1 modify for shorten the time after powered up 20150428 end*/
...
/*lenovo-sw caoyi1 add for gsensor calibration interface attributes 20150420 begin*/
	acceld->cdev.sensors_calibrate = kxtj9_sensors_calibrate_set;
	acceld->cdev.sensors_self_test = kxtj9_sensors_self_test;
/*lenovo-sw caoyi1 add for gsensor calibration interface attributes 20150420 end*/
...
/*lenovo-sw caoyi1 add for close LDO after probe finish 20150417 begin*/
	if (accel_pdata->acc_power_on)
		err = accel_pdata->acc_power_on(false);
/*lenovo-sw caoyi1 add for close LDO after probe finish 20150417 end*/
