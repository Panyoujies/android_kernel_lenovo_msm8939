	...
	int hard)
{
	...
	if (hard != 0) {
/*lenovo-sw caoyi1 modify for Msensor gpio_rstn set output 20150414 begin*/
		if(gpio_is_valid(akm->gpio_rstn))
		{
			printk("%s  gpio_is_valid OK\n",__func__);

			gpio_request(akm->gpio_rstn,"akm09911_gpio_rstn");
...
/*lenovo-sw caoyi1 modify for Msensor gpio_rstn set output 20150414 end*/
		}
...
static int akm_report_data(struct akm_compass_data *akm)
{
	...
	akm->last_y = mag_y;
	akm->last_z = mag_z;
	/* Begin, lenovo-sw youwc1: for msensor raw data */
	akm->cdev.raw_data.x = mag_x;
	akm->cdev.raw_data.y = mag_y;
	akm->cdev.raw_data.z = mag_z;
	/* End, lenovo-sw youwc1: for msensor raw data */
	input_sync(akm->input);
