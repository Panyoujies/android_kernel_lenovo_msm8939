...
/*+Begin xujt1 add for obtaining module id 2015-05-27*/
			if(rc != -EAGAIN){/*make original try logic*/
			        msleep(20);
			        continue;
			}else{
			        /*it shows that sensor id match but module ID doesn't match, 
			           just break to avoid following try option, which will result in 
			           increasing power up time.*/
			        break;
			}
/*+End xujt1 add for obtaining module id 2015-05-27*/
		} else {
			break;
		}
	}
...
/*+Begin: huangsh4 for imx179 match id. */
    if (strstr(sensor_name, "s5k2p8") != NULL) {
	...
	/*+Begin xujt1 add for obtaining module id 2015-05-27*/
	    ...
	    if (mid != slave_info->module_id) {
                  pr_err("%s: module Id doesn't match: module id: %d,equired mid: %d\n", __func__, mid, slave_info->module_id);
    		return -EAGAIN;/*return dedicate error value to show module ID doesn't match*/
             }
	}
	/*+End xujt1 add for obtaining module id 2015-05-27*/
        return rc;
...
int hi545_otp_read_module_info(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (checksum != hi545_otp_module_info[MODULE_INFO_GROUP_SIZE-1]) {
		CDBG("%s: module info checksum mismatch!!! calc_val: %x, read_val: %x\n", __func__, checksum, hi545_otp_module_info[MODULE_INFO_GROUP_SIZE-1]);
		return -2;
	} else {
		CDBG("%s: module info checksum match success!!! val: %x\n", __func__, checksum);
	}
