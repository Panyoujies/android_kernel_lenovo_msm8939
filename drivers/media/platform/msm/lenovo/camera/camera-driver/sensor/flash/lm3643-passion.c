...
int lm3642_flash_current_to_reg(uint32_t current_in_ma, uint32_t led_idx)
{
	...
	uint8_t flash_level = (current_in_ma*1000-10900)/11725;
	...
	if (flash_level) {
		lm3642_high_array[0].reg_data |= (0x01<<led_idx);
		lm3642_high_array[led_idx+1].reg_data = flash_level;
	} else {
		lm3642_high_array[0].reg_data &= ~(0x01<<led_idx);
	}
...
{
	uint8_t torch_level = (current_in_ma*1000-977)/1400;
	...
	if (torch_level) {
		lm3642_low_array[0].reg_data |= (0x01<<led_idx);
		lm3642_low_array[led_idx+1].reg_data = torch_level;
	} else {
		lm3642_low_array[0].reg_data &= ~(0x01<<led_idx);
	}
...
int lm3642_flash_duration_set(uint32_t ms, uint32_t led_idx)
{
	...
	if (ms <= 100) {
		lm3642_high_array[3].reg_data = (ms/10 + (ms%10? 1 : 0))-1;
	} else {
		lm3642_high_array[3].reg_data = ((ms-100)/50 + ((ms-100)?1:0))+9;
	}
