...
/*+begin ljk add led flash driver 2014-06-20*/
#include <linux/proc_fs.h>
/*+end ljk add led flash driver 2014-06-20*/
...
uint16_t sid = 0;
#ifdef LM3643A
#define LM3643A_SID 0x67
#endif 
/*+end ljk add led flash driver 2014-06-20*/
/*+begin huangsh4 add led flash driver 2014-11-27*/
extern struct msm_led_flash_ctrl_t fctrl_max;
extern struct max77819_flash *flash_max77819;
extern int max77819_flash_led_init(struct msm_led_flash_ctrl_t *fctrl_max,
	struct max77819_flash *flash);
extern int max77819_flash_led_low(struct msm_led_flash_ctrl_t *fctrl_max,
	struct max77819_flash *flash);
extern int max77819_flash_led_high(struct msm_led_flash_ctrl_t *fctrl_max,
	struct max77819_flash *flash);
extern int max77819_flash_led_low_ext(struct msm_led_flash_ctrl_t *fctrl_max,
	struct max77819_flash *flash,uint32_t torch_current);
extern int max77819_flash_led_release(struct msm_led_flash_ctrl_t *fctrl_max,
	struct max77819_flash *flash);
extern int max77819_flash_led_off(struct msm_led_flash_ctrl_t *fctrl_max, 
	struct max77819_flash *flash);
extern int max77819_flash_led_high_ext(struct msm_led_flash_ctrl_t *fctrl_max,
	struct max77819_flash *flash,uint32_t flash_current);
extern uint8_t lm3643_torch_current_to_dac(uint32_t torch_current);
extern uint8_t lm3643_flash_current_to_dac(uint32_t flash_current);
/*+end huangsh4 add led flash driver 2014-11-27*/
...
/*+end ljk add led flash driver 2014-06-20*/
	...
	CDBG("%s:%d called\n", __func__, __LINE__);
	flash_max77819 = platform_get_drvdata(fctrl_max.pdev);
...
/*begin add flash power setting*/
	if ( fctrl->vreg_cfg.num_vreg <= MSM_VREG_MAX_VREGS) {
	    for (i = 0; i <  fctrl->vreg_cfg.num_vreg; i++) {
		rc = msm_camera_config_single_vreg(&(fctrl->pdev->dev),
			&fctrl->vreg_cfg.cam_vreg[i],
			(struct regulator **)&fctrl->vreg_cfg.data[i],
			1);
	    }
	}
/*end add flash power setting*/
...
/*begin set flash lm3642a by ljk*/
		rc = fctrl->flash_i2c_client->i2c_func_tbl->i2c_read(
			fctrl->flash_i2c_client,
			fctrl->flashdata->slave_info->sensor_id_reg_addr,
			&chipid,MSM_CAMERA_I2C_BYTE_DATA);
		CDBG("%s:%d  flash led 1 sid = 0x%x,id_reg_addr = 0x%x,id = 0x%x\n", __func__, __LINE__,
				fctrl->flash_i2c_client->cci_client->sid,
				fctrl->flashdata->slave_info->sensor_id_reg_addr,
				fctrl->flashdata->slave_info->sensor_id);
		if (rc < 0)
			pr_err("%s:%d  read led flash chip failed\n", __func__, __LINE__);
		else
			CDBG("%s:%d  led flash chipid  = %d \n", __func__, __LINE__, chipid);
		#ifdef LM3643A 
		sid	= fctrl->flash_i2c_client->cci_client->sid;
		fctrl->flash_i2c_client->cci_client->sid = LM3643A_SID;
		rc = fctrl->flash_i2c_client->i2c_func_tbl->i2c_read(
			fctrl->flash_i2c_client,
			fctrl->flashdata->slave_info->sensor_id_reg_addr,
			&chipid,MSM_CAMERA_I2C_BYTE_DATA);
		CDBG("%s:%d  flash led 2 sid = 0x%x,id_reg_addr = 0x%x,id = 0x%x\n", __func__, __LINE__,
				fctrl->flash_i2c_client->cci_client->sid ,
				fctrl->flashdata->slave_info->sensor_id_reg_addr,
				fctrl->flashdata->slave_info->sensor_id);
		if (rc < 0)
			...
			CDBG("%s:%d  led flash chipid  = %d \n", __func__, __LINE__, chipid);
		fctrl->flash_i2c_client->cci_client->sid = sid;
		#endif
/*end set flash lm3642a by ljk*/
	}
    i2c_inited = 1;
/*+end ljk add led flash driver 2014-06-20*/
...
/*end set flash lm3642a by ljk*/
	...
	rc = max77819_flash_led_init(&fctrl_max, flash_max77819);
	return rc;
}
...
/*end add flash power setting*/
	rc = max77819_flash_led_release(&fctrl_max, flash_max77819);
	fctrl->led_state = MSM_CAMERA_LED_RELEASE;
	/* CCI deInit */
	if (fctrl->flash_device_type == MSM_CAMERA_PLATFORM_DEVICE) {
		rc = fctrl->flash_i2c_client->i2c_func_tbl->i2c_util(
...
int msm_flash_led_off(struct msm_led_flash_ctrl_t *fctrl)
{
	...
	flashdata = fctrl->flashdata;
	power_info = &flashdata->power_info;
	rc = max77819_flash_led_off(&fctrl_max, flash_max77819);
	CDBG("%s:%d called\n", __func__, __LINE__);
	if (fctrl->flash_i2c_client && fctrl->reg_setting) {
		rc = fctrl->flash_i2c_client->i2c_func_tbl->i2c_write_table(
			fctrl->flash_i2c_client,
	...
	gpio_set_value_cansleep(
		...
		GPIO_OUT_HIGH);
         ...
         fctrl->reg_setting->low_setting->reg_setting[0].reg_data =
		 	lm3643_torch_current_to_dac(fctrl->torch_op_current[0]);
         fctrl->reg_setting->low_setting->reg_setting[1].reg_data =
		 	lm3643_torch_current_to_dac(fctrl->torch_op_current[1]);
...
/*end set flash lm3642a by ljk*/
	...
	gpio_set_value_cansleep(
		...
		GPIO_OUT_LOW);
/*+end ljk add led flash driver 2014-06-20*/
	rc = max77819_flash_led_low(&fctrl_max, flash_max77819);
	return rc;
}
...
{
    CDBG("%s:%d called\n", __func__, __LINE__);
	CDBG("%s:%d shen called ****flash_current = %d \n", __func__, __LINE__,flash_current);
	/*if (flash_current < 25) {     
	           flash_current = 24; 
	 } else if (25 <= flash_current && flash_current < 100){
			   flash_current = 500;
	  } else if (100 <= flash_current && flash_current < 225){
			   flash_current = 450;
	  } else if (225 <= flash_current && flash_current < 900){
			   flash_current = 350;
	  } else if (1000 <= flash_current && flash_current < 1200){
			   flash_current = 200;
	  } else if (1200 <= flash_current){
			   flash_current = 24;
	  }*/
	flash_current = 200;
...
{
	int rc = 0;
	uint32_t max77819_current;
	struct msm_camera_sensor_board_info *flashdata = NULL;
	struct msm_camera_power_ctrl_t *power_info = NULL;
	CDBG("%s:%d called\n", __func__, __LINE__);
	max77819_current = select_max77819_flash_current(fctrl->flash_op_current[1]);
	CDBG("%s:%d called shen after max77819_current = %d \n", __func__, __LINE__,max77819_current);
    CDBG("%s:%d called shen led1 = %d ,led2 = %d \n", __func__, __LINE__,fctrl->flash_op_current[0],fctrl->flash_op_current[1]);
	flash_max77819 = platform_get_drvdata(fctrl_max.pdev);
	rc = max77819_flash_led_high_ext(&fctrl_max, flash_max77819,max77819_current);
	flashdata = fctrl->flashdata;
	power_info = &flashdata->power_info;	 
/*+begin ljk add led flash driver 2014-06-20*/
	gpio_set_value_cansleep(
	...
	gpio_set_value_cansleep(
		...
		GPIO_OUT_LOW);
         ...
         fctrl->reg_setting->high_setting->reg_setting[0].reg_data =
		 	lm3643_flash_current_to_dac(fctrl->flash_op_current[0]); 
         fctrl->reg_setting->high_setting->reg_setting[1].reg_data =
		 	lm3643_flash_current_to_dac(fctrl->flash_op_current[1]); 
...
/*begin add flash power by ljk*/
	...
	{
		fctrl->torch_max_current[count] = 179;
		fctrl->flash_max_current[count] = 1500;
	}
	...
	fctrl->flash_max_current[count] = 0;
        	...
        	if (of_find_property(of_node,
        		...
        		rc = msm_camera_get_dt_vreg_data(of_node,
                	...
                	if (rc < 0)
        	        	CDBG("msm_camera_get_dt_vreg_data fail \n");
        	}
/*end add flash power by ljk*/
		return rc;
ERROR9:
...
ssize_t  proc_flash_led_write (struct file *file, const char __user *buf, size_t nbytes, loff_t *ppos)
{
    ...
    {
    		CDBG("ljk MSM_CAMERA_LED1_LOW called\n");

	   if (fctrl->func_tbl->flash_led_release)
       rc = fctrl->func_tbl->flash_led_release(fctrl);
	   	if (rc) {
		pr_err("flash_led1_release failed rc %d\n", rc);
		...
		if (fctrl->func_tbl->flash_led_init)
		{
            ...
            {
                if ((fctrl->func_tbl->flash_led1_low)&&(i2c_inited==1))
/*+begin add color temp for flash by ljk*/
    	            rc = fctrl->func_tbl->flash_led1_low(fctrl);
/*+end*/
    		}
    	}
    	else
	...
	else if (!strcmp((const char *)string, (const char *)"led2_on"))
    {
	   ...
	   CDBG("ljk MSM_CAMERA_LED2_LOW called\n");	
	   if (fctrl->func_tbl->flash_led_release)
       rc = fctrl->func_tbl->flash_led_release(fctrl);
	   	if (rc) {
		pr_err("flash_led2_release failed rc %d\n", rc);
		...
		if (fctrl->func_tbl->flash_led_init)
		{
            ...
            {
                if ((fctrl->func_tbl->flash_led2_low)&&(i2c_inited==1))
/*+begin add color temp for flash by ljk*/
    	            rc = fctrl->func_tbl->flash_led2_low(fctrl);
/*+end*/
    		}
    	}
    	else
	...
	else if (!strcmp((const char *)string, (const char *)"led3_on"))
    {
    		CDBG("ljk MSM_CAMERA_LED3_LOW called\n");
	   if (fctrl->func_tbl->flash_led_release)
       rc = fctrl->func_tbl->flash_led_release(fctrl);
	   	if (rc) {
		pr_err("flash_led3_release failed rc %d\n", rc);
		...
		if (fctrl->func_tbl->flash_led_init)
		{
            ...
            {
                if ((fctrl->func_tbl->flash_led3_low)&&(i2c_inited==1))
/*+begin add color temp for flash by ljk*/
    	            rc = fctrl->func_tbl->flash_led3_low(fctrl);
/*+end*/
    		}
    	}
    	else
	...
	else if (!strcmp((const char *)string, (const char *)"all_on"))
    {
       ...
       rc = fctrl->func_tbl->flash_led_release(fctrl);
		...
		if (fctrl->func_tbl->flash_led_init)
		{
            ...
            {
                if ((fctrl->func_tbl->flash_led_low)&&(i2c_inited==1))
/*+begin add color temp for flash by ljk*/
    	            rc = fctrl->func_tbl->flash_led_low(fctrl);
/*+end*/
    		}
