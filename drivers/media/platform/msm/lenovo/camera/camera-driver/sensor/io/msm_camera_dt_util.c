...
/*+begin add for s5k2p8 evdd by ljk*/
		case CAM_EVDD:
			for (j = 0; j < num_vreg; j++) {
				if (!strcmp(cam_vreg[j].reg_name, "cam_evdd")) {
					pr_err("%s:%d i %d j %d cam_vana\n",
						__func__, __LINE__, i, j);
					power_setting[i].seq_val = j;
					break;
			...
			break;
/*+end add for s5k2p8 evdd by ljk*/
...
/*+begin xujt1 add VCM driver for front camera 2014-07-03*/
		case CAM_VAF_F:
			for (j = 0; j < num_vreg; j++) {
				if (!strcmp(cam_vreg[j].reg_name, "cam_vaf_front")) {
					pr_err("%s:%d i %d j %d cam_vaf_front\n", __func__, __LINE__, i, j);
					power_setting[i].seq_val = j;
					break;
				}
			}
			break;
/*+end xujt1 add VCM driver for front camera 2014-07-03*/	
...
/*+Begin xujt1 revert the gpio configure value for power_down_setting 2015-03-20*/
        {
		int index = 0;
		struct msm_sensor_power_setting *pd = NULL;
		for (index = 0; index < power_info->power_down_setting_size; index++) {
		    pd = &power_info->power_down_setting[index];
		    if(pd->seq_type == SENSOR_GPIO){
			if(pd->config_val == GPIO_OUT_HIGH){
				pd->config_val = GPIO_OUT_LOW;
			}else if(pd->config_val == GPIO_OUT_LOW){
				pd->config_val = GPIO_OUT_HIGH;
			}
		    }
		}
        }
/*+End xujt1 revert the gpio configure value for power_down_setting 2015-03-20*/
	return rc;
ERROR2:
	kfree(array);
ERROR1:
...
/*+begin xujt1 add gpio-vio dts parse*/
	rc = of_property_read_u32(of_node, "qcom,gpio-vio", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {              
			pr_err("%s:%d read qcom,gpio-vio failed rc %d\n", __func__, __LINE__, rc);
		...
		gconf->gpio_num_info->valid[SENSOR_GPIO_VIO] = 1;
		CDBG("%s qcom,gpio-vio %d\n", __func__, gconf->gpio_num_info->gpio_num[SENSOR_GPIO_VIO]);
	} else {
		rc = 0;
	}	  
/*+end xujt1 add gpio-vio dts parse*/
    ...
    /*begin + add for camera common power*/
	rc = of_property_read_u32(of_node, "qcom,gpio-reset-f", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-reset-f failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-reset-f invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_RESET_F] =
			gpio_array[val];
		gconf->gpio_num_info->valid[SENSOR_GPIO_RESET_F] = 1;
		CDBG("%s qcom,gpio-reset-f %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_RESET_F]);
	} else {
		rc = 0;
	}
	rc = of_property_read_u32(of_node, "qcom,gpio-vdig-f", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-vdig-f failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-vdig-f invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_VDIG_F] =
			gpio_array[val];
		gconf->gpio_num_info->valid[SENSOR_GPIO_VDIG_F] = 1;
		CDBG("%s qcom,gpio-vdig-f %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_VDIG_F]);
	} else {
		rc = 0;
	}
	/*end + add for camera common power*/
	return rc;
