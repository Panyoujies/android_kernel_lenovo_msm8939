...
static int32_t msm_actuator_init_lc_tune_data(struct msm_actuator_ctrl_t *a_ctrl, struct msm_actuator_set_info_t *set_info)
{
    ...
    rc = IniSetTop();// LC898122 OIS initialize
    CDBG("%s IniSet return %d\n", __func__,rc);

    RtnCenTop(0x00);// Lens centering
	...
	void __user *argp)
{
	...
	case CFG_SET_OISMODE:
        ...
        if (OIS_STATUS == 1) {
            ...
            if (cam_mode == 1) //video
        	    SetH1cModTop(0xff); /* Lvl Change Active mode */
        	else               //preview
       	        SetH1cModTop(0x00);
    	}
		break;
...
extern int vl6180_i2c_read_32bits(unsigned int addr,   uint32_t * ReadData );
...
static ssize_t laser_show_rtnrate(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (LASER_STATUS == 1) {
        ...
        /*+Begin xujt1 add invalid checking and do correction 2015-07-06*/
	vl6180_i2c_read_32bits( 0x006c, &signal);
	if(signal & 0x80000000)
	{
                  pr_err("%s (0x6c)ignal = 0x%x\n",__func__,signal);
		rtnrate = 0;
	}
        /*+End xujt1 add invalid checking and do correction 2015-07-06*/
