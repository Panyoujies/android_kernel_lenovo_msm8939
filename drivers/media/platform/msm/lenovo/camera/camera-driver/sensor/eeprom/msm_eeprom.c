...
extern int	E2pDat_Primax_Lenovo( uint8_t * memory_data);
...
int msm_eeprom_get_module_id(const char *eeprom_name)
{
    ...
    IS_E2PROM_ONSEMI(eeprom_name)  {
        if (is_3a_checksumed && eeprom_rear_data_ctrl != NULL) {
            return (eeprom_rear_data_ctrl->cal_data.mapdata[2] << 8)  |eeprom_rear_data_ctrl->cal_data.mapdata[3];
        }
    }
...
/*+end lijk3 add eeprom checksum function 2014-07-11*/
	...
	else if (!strcmp(eb_info->eeprom_name, E2PROM_ONSEMI))
	{
    	...
    	E2pDat_Primax_Lenovo(eeprom_rear_data_ctrl->cal_data.mapdata);
